...
#region Get Actual type
        ...
        internal override Type Internal_GetActualType(object obj)
        {
            ...
            //if (type.Assembly.GetName().Name.Contains("InjectedMonoTypes")) return type;
            try
            {
...
#region Deobfuscation cache
        ...
        private static List<string> AlreadyChecked = new();
        private static void BuildDeobfuscationCache()
        {
            foreach (Assembly asm in AppDomain.CurrentDomain.GetAssemblies())
            {
                if (AlreadyChecked.Contains(asm.FullName))
                    continue;
                AlreadyChecked.Add(asm.FullName);
            ...
            try
            {
                ...
                foreach (CustomAttributeData att in type.CustomAttributes)
                {
                    if (att.AttributeType == typeof(ObfuscatedNameAttribute))
                    {
                        ...
                        if (!obfuscatedToDeobfuscatedTypes.ContainsKey(obfuscatedName))
                            obfuscatedToDeobfuscatedTypes.Add(obfuscatedName, type);
                        if (!deobfuscatedToObfuscatedNames.ContainsKey(type.FullName))
