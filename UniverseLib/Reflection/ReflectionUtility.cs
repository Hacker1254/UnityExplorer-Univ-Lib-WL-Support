...
namespace UniverseLib
{
    ...
    public class ReflectionUtility
    {
        ...
        {
            foreach (var file in Directory.GetFiles("WorldLoader\\UnhollowedAsm"))
                if (file.EndsWith(".dll"))
                    CacheTypes(Assembly.Load(File.ReadAllBytes(file)));
            foreach (Assembly asm in AppDomain.CurrentDomain.GetAssemblies())
                CacheTypes(asm);
        }

        private static List<string> LoggedAsms = new();

        internal static void CacheTypes(Assembly asm)
        {

            try {
                if (LoggedAsms.Contains(asm.FullName)) return;
                LoggedAsms.Add(asm.FullName);
        ...
        public static IEnumerable<Type> TryGetTypes(Assembly asm)
        {
            try {
                ...
                try {
                    ...
                    return from t in ex.Types where t != null select t;
                }
            }
            catch {
            }
